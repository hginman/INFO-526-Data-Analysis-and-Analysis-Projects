...
ggplot(top_categories, aes(x = reorder(Category.Name, TotalSales), y = TotalSales)) + 
  ...
  labs(title = "Top 10 Alcohol Categories by Sales",
       x = NULL,
       y = "Total Sales ($)") +
  theme_minimal() +
...
ggplot(worst_categories, aes(x = reorder(Category.Name, -TotalSales), y = TotalSales)) + 
  ...
  labs(title = "Bottom 10 Alcohol Categories by Sales",
       x = NULL,
       y = "Total Sales ($)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
ggplot(summed_sales, aes(x = interaction(Year, Month, sep = "-"), y = Total.Sales, color = Category.Name)) +
  ...
  geom_point() +
  labs(title = "Total Sales Over Time by Category", x = "Month and Year", y = "Total Sales") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_y_continuous(labels = scales::label_number(scale = 1e-6, suffix = "M"))
...
american_vodka_sales <- iowa_sales %>%
  ...
  summarize(
    TotalVolume = sum(Volume.Sold.Liters),
    TotalRevenue = sum(Bottles.Sold * State.Bottle.Retail)
  ) %>%
  arrange(-TotalVolume)
...
ggplot(filtered_vodka_sales, aes(x = reorder(Item.Description, TotalVolume), y = TotalVolume)) +
  ...
  theme_minimal() +
  labs(
    title = "Sales of Selected American Vodkas",
    x = NULL,
    y = "Total Sales Volume (Liters)") +
...
least_revenue_alcohol <- head(least_revenue_alcohol, 20)

ggplot(least_revenue_alcohol, aes(x = Item.Description, y = TotalRevenue)) +
  geom_point(color = "red") + 
  coord_flip() + 
  labs(
    title = "Alcohol Products with Least Revenue",
    x = "Item Description",
    y = "Total Revenue"
  ) +
  theme_minimal() +
